...
int count=0,j=0, faults = 0;
uint16_t channels[8];  // array for raw channel values 
uint16_t  motor_val[4]={1000,1000,1000,1000}; // array with motor inputs: [F,B,L,R]
uint32_t timer = 0;
uint16_t safe_tick = 0;
uint16_t rcthr, rcyaw, rcpit, rcroll,rcdebug;   // Variables to store rc input
float mroll = 0, mpitch = 0, myaw = 0;
float Thrust = 0, Roll = 0, Pitch = 0, Yaw = 0, v_dot_k=0, alpha =0, debug = 0;
Quaternion_D q_1,q_2,q_3,q_4,q_5,q_z,q_xy,q_k;
Vector3<float> tau, omega_k,T_k, T_star, lin_T[5], debug_T;
Matrix3<float> D_k;
bool trigger = true, first_tick = true;
bool is_safe = true, prev_safe = 1;
...
void loop()
{
  ...
  if(is_safe){
    ...
    //if (ins.num_samples_available() >= 1) {
      ...
      q_1.from_euler(ahrs.roll,ahrs.pitch,0);//ahrs.pitch,ahrs.yaw);//ahrs.roll,ahrs.pitch,ahrs.yaw); // current attitude
      ...
      q_2.from_euler(ToRad(Roll),ToRad(Pitch),0);
      ...
      //hal.console->printf_P(PSTR("[%d,%d,%d]\r\n"),rcthr,rcpit,rcroll);
      /*if(Roll > 0 && first_roll){
            first_roll = false;
            first_tick = true;
            v_dot_k = 0;
            T_k.x = 0;
      ...
      if(Thrust > 0 ){
        ...
        /* UNCOMMENT TO CHOOSE TRIGGERING STRATEGY: TRIG_1: ANALITIC TRIGGERING (SAFE);
                                                    TRIG 2: APPROX TRIGGERING (SAFE, IF alpha_2 IS A LOW VALUE IN saturating_controller.h
                                                    TRIG 3: HEURISTIC TRIGGERING (Same as trig2, for alpha_3)
                                                    trigger = true: PERIODIC TRIGGERING */
        //trigger = trig_func1(q_1,q_2,omega,D_k,omega_k,T_k,v_dot_k, &alpha, &T_star);
        //trigger = trig_func2(omega,D_k,omega_k,lin_T,v_dot_k, q_1, q_2, q_k, &T_star, &debug);
        //trigger = trig_func3(omega,omega_k, q_1, q_2,q_k);
        trigger = true;
        ...
        if(safe_tick > safe_lim) // Abort
          is_safe = false;
        
        if(trigger){


          tau = fast_and_saturating_controller(&q_1,&q_2,&omega, &D_k, &T_k,&q_xy,&q_z);
          ...
          //hal.console->printf_P(PSTR("v: [%.7f]\r\n"),v_dot_k);
        }
      ...
      //motor_val[MOTOR_F] = rcthr;
      //motor_val[MOTOR_B] = rcthr;
      //motor_val[MOTOR_L] = rcthr;
      //motor_val[MOTOR_R] = rcthr;
     ...
     //UNCOMMENT BLOCK TO ENABLE LOGGING AFTER ROLL > VALUE (allows to log step responses)
    /* if(Roll > 10){
